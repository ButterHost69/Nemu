...
func VerifySessionToken(db *sql.DB, sessionToken string) (bool, error) {
	...
	fmt.Println("The token is : " + sessionToken)
...
func CreateSessionTokens(db *sql.DB, userName string) (bool, string) {
	...
	// Check User in Session Table
	if checkIfUserExists(db, userName) {
		database.DeleteSessionFromSessionTokenTableUsingUsername(db, userName)
	}
...
func DeleteSession(db *sql.DB, sessionToken string) {
	...
	if exists {
		database.DeleteSessionFromSessionTokenTableUsingSessionID(db, sessionToken)
	}
}
